(define (domain restaurante-dom)
  ...
  (:predicates
    ...
    ;; bandejas pueden llevar cualquier objeto ubicable (bocadillos o ingredientes)
    (en-bandeja ?obj - ubicable ?t - bandeja)
    (servido ?p - persona)
  ...
  (:action poner_en_bandeja
    :parameters (?obj - ubicable ?t - bandeja ?k - cocina)
    :precondition (and (en ?obj ?k) (en ?t ?k))
    :effect (and
      (not (en ?obj ?k))
      (en-bandeja ?obj ?t)
    )
  ...
  ;; cuando una bandeja que contiene ingredientes individualess llega a una cocina,
  ;; dichos ingredientes se pueden depositar en la cocina para que sean usados más tarde
  (:action depositar_ingrediente
    :parameters (?ing - (either porcion-pan porcion-relleno) ?t - bandeja ?k - cocina)
    :precondition (and
      (en-bandeja ?ing ?t)
      (en ?t ?k)
    )
    :effect (and
      (not (en-bandeja ?ing ?t))
      (en ?ing ?k)
    )
  )
)